...
static void __iomem *anatop_base = VF610_ANADIG_BASE_ADDR;
static void __iomem *ccm_base = VF610_CCM_BASE_ADDR;
...
static void __init vf610_clocks_init(struct device_node *ccm_node)
{
	...
	clk[VF610_CLK_SXOSC] = clk_fixed("sxosc", 32768);
	clk[VF610_CLK_FXOSC] = clk_fixed("fxosc", 24000000);
	clk[VF610_CLK_AUDIO_EXT] = clk_fixed("audio_ext", AUDIO_EXTERNAL_CLK);
	clk[VF610_CLK_ENET_EXT] = clk_fixed("enet_ext", ENET_EXTERNAL_CLK);

	/* Clock source from external clock via LVDs PAD */
	clk[VF610_CLK_ANACLK1] = clk_fixed(ccm_node, "anaclk1",);
	...
	clkdev_add_physbase(clks[VF610_CLK_UART0], VF610_UART0_BASE_ADDR, NULL);
	clkdev_add_physbase(clks[VF610_CLK_UART1], VF610_UART1_BASE_ADDR, NULL);
	clkdev_add_physbase(clks[VF610_CLK_UART2], VF610_UART2_BASE_ADDR, NULL);
	clkdev_add_physbase(clks[VF610_CLK_UART3], VF610_UART3_BASE_ADDR, NULL);
	clkdev_add_physbase(clks[VF610_CLK_UART4], VF610_UART4_BASE_ADDR, NULL);
	clkdev_add_physbase(clks[VF610_CLK_UART5], VF610_UART5_BASE_ADDR, NULL);
	clkdev_add_physbase(clks[VF610_CLK_I2C0], VF610_I2C0_BASE_ADDR, NULL);
	clkdev_add_physbase(clks[VF610_CLK_I2C1], VF610_I2C1_BASE_ADDR, NULL);
	clkdev_add_physbase(clks[VF610_CLK_I2C2], VF610_I2C2_BASE_ADDR, NULL);
	clkdev_add_physbase(clks[VF610_CLK_I2C3], VF610_I2C3_BASE_ADDR, NULL);
	clkdev_add_physbase(clks[VF610_CLK_ENET0], VF610_ENET_BASE_ADDR, NULL);
	clkdev_add_physbase(clks[VF610_CLK_ENET1], VF610_ENET_BASE_ADDR1, NULL);
	clkdev_add_physbase(clks[VF610_CLK_ESDHC0], VF610_ESDHC0_BASE_ADDR, NULL);
	clkdev_add_physbase(clks[VF610_CLK_ESDHC1], VF610_ESDHC1_BASE_ADDR, NULL);
	clkdev_add_physbase(clks[VF610_CLK_OCOTP], VF610_OCOTP_BASE, NULL);
	clkdev_add_physbase(clks[VF610_CLK_PIT], VF610_PIT_BASE_ADDR, NULL);
}

static int vf610_ccm_probe(struct device_d *dev)
{
	...
	vf610_clocks_init(dev, regs);

	return 0;
}

static __maybe_unused struct of_device_id vf610_ccm_dt_ids[] = {
	{
...
static struct driver_d imx53_ccm_driver = {
	.probe	= vf610_ccm_probe,
	.name	= "vf610-ccm",
	.of_compatible = DRV_OF_COMPAT(imx53_ccm_dt_ids),
};

static int vf610_ccm_init(void)
{
	if (IS_ENABLED(CONFIG_ARCH_VF610))
		platform_driver_register(&vf610_ccm_driver);

	return 0;
}
core_initcall(vf610_ccm_init);
